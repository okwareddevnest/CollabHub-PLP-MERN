const taskSocket = (taskNamespace) => {
    taskNamespace.on('connection', (socket) => {
      console.log(`✅ New connection to /tasks namespace: ${socket.id}`);
  
      /**
       * 1️⃣ User joins a specific task board
      ...
      socket.on('joinTaskBoard', (boardId) => {
        socket.join(boardId);
        console.log(`➡️ User ${socket.id} joined task board: ${boardId}`);
      });
  
      /**
       * 2️⃣ Handle New Task creation
      ...
      socket.on('taskCreated', (taskData) => {
        if (taskData.boardId) {
          ...
          taskNamespace.to(taskData.boardId).emit('taskCreated', taskData);
      ...
      /**
       * 4️⃣ Handle Task deletion
       */
      socket.on('taskDeleted', (taskData) => {
        if (taskData.boardId) {
          taskNamespace.to(taskData.boardId).emit('taskDeleted', taskData);
          console.log(`🗑️ Task deleted on board ${taskData.boardId}`);
        } else {
